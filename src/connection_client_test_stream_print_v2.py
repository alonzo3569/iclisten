...
while a< 10000000000000:
    ...
    # Process data message
      ...
      size = len(volt_list)
      #print(f'{size}')
      if size > 476*100: #80 good
        current_frame[0:(-size)] = previous_frame[size:] # index 0 to index -1 = index 1 to last element
        current_frame[(-size):] = volt_list
        ...
        plt.plot(current_frame, linewidth=0.5)
        plt.pause(0.00000000001)
        volt_list = []
        #print(f'current_frame: {len(current_frame)}')
        #print(f'previous_frame: {len(previous_frame)}')
